...
class EXPS:
    # 1.3665368098726338
    kickx_polyexps = [(1, 0), (3, 0)]

    # 0.34169065724667325
    kicky_polyexps = [(0, 1), (2, 1)]
...
def get_kickmap_fit(kmap, fname, exps_to_remove, nrpts,
                    plot_flag=False, save_map=False, sufix='-filtered'):
    ...
    if save_map:
        sufix_ = sufix + '.txt'
        fname = fname.replace('.txt', sufix_)
...
if __name__ == '__main__':
    ...
    fname = Tools._get_kmap_filename(
        width=widths[0], gap=gaps[1], phase=phases[0], filter_flag=False,
        shift_flag=True)
    kmap = IDKickMap(kmap_fname=fname)

    # do_fitting(iter=15, kick_component='x')
    # do_fitting(iter=15, kick_component='y')

    nrpts = 101
    exps_to_remove = {'x': [()],
                      'y': [(0, 1), (2, 1)]}
    get_kickmap_fit(kmap, fname, exps_to_remove, nrpts, False,
                    save_map=True, sufix='-filtered')
